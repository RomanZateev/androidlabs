...
public class Lab3Activity extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (item.getItemId() == R.id.action_filter) {
            ...
            builder.setMessage(R.string.filter)
...
//            finish();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_STUDENT_ADD && resultCode == RESULT_OK) {
            Student student = TestActivity.getResultStudent(data);

            studentsCache.addStudent(student);

            studentsAdapter.setStudents(studentsCache.getStudents());
            studentsAdapter.notifyItemRangeInserted(studentsAdapter.getItemCount(), 1);
            list.scrollToPosition(studentsAdapter.getItemCount());
        }
    }
}
